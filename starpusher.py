        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == QUIT:
                ...
                # Set the camera move mode.
                    ...
                    # Reset the player image to the first one.
                    currentImage = 0
                    mapNeedsRedraw = True
                elif event.key == K_r:
                    return 'reset'  # Reset the level.
...
def readLevelsFile(filename):
    ...
    for lineNum in range(len(content)):
        ...
        if line != '':
            ...
            assert startx is not None and starty is not None, 'Level %s (around line %s) in %s is missing a "@" or "+" to mark the start point.' % (
    ...
    # Shuffle the levels list to select a random level next
...
def isLevelFinished(levelObj, gameStateObj):
    ...
    for goal in levelObj['goals']:
        if goal not in gameStateObj['stars']:
            # Found a goal without a star on it.
            return False
    return True
