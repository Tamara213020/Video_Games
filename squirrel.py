...
LOSTSIZE = 10        # how small the player needs to be to lose
WINWIDTH = 960  # зголеми го ширината на прозорецот на 960 пиксели
WINHEIGHT = 480 # останува иста висината на прозорецот на 480 пиксели
# added or modified to provide functionality
BONUS_SPEED = 5  # Бонус брзина при притискање на "b"
...
CAMERASLACK_HORIZONTAL = 90   # хоризонтална насока, колку далеку од центарот камерата да се движи катерото пред да се движи камерата
CAMERASLACK_VERTICAL = 90     # вертикална насока, колку далеку од центарот на камерата да се движи катерото пред да се движи камерата
...
def runGame():
    global MOVERATE  # Додадено за да може променливата да се промени во функцијата
    ...
    while True: # main game loop
        ...
        for sObj in squirrelObjs:
            ...
            if random.randint(0, 99) < DIRCHANGEFREQ:
                ...
                sObj['bounceDirection'] = random.choice([UP, DOWN])
        ...
        if (camerax + HALF_WINWIDTH) - playerCenterx > CAMERASLACK_HORIZONTAL:
            camerax = playerCenterx + CAMERASLACK_HORIZONTAL - HALF_WINWIDTH
        elif playerCenterx - (camerax + HALF_WINWIDTH) > CAMERASLACK_HORIZONTAL:
            camerax = playerCenterx - CAMERASLACK_HORIZONTAL - HALF_WINWIDTH
        if (cameray + HALF_WINHEIGHT) - playerCentery > CAMERASLACK_VERTICAL:
            cameray = playerCentery + CAMERASLACK_VERTICAL - HALF_WINHEIGHT
        elif playerCentery - (cameray + HALF_WINHEIGHT) > CAMERASLACK_VERTICAL:
            cameray = playerCentery - CAMERASLACK_VERTICAL - HALF_WINHEIGHT
...
def makeNewSquirrel(camerax, cameray):
    ...
    # Додајте го 'bounceDirection'
    sq['bounceDirection'] = random.choice([UP, DOWN])
